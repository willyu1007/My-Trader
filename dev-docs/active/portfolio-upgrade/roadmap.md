...
- **标的数据源：单一数据源（active）**。同一标的在同一数据域（行情/公司行为/汇率/日历等）只允许一个“当前生效”的数据源；更换数据源视为“替换数据”，而不是新增一个标的或并行保留多套数据。
  - 切换规则：切换后应清理该标的旧数据域缓存（避免跨日期混用不同 source），再按新数据源重新拉取/回填；所有数据保留 `source/ingested_at/run_id` 以便追溯。
- **分析/机会发现的标签语义：行业 + 主题 两个 Facet**。底层继续以 tags 承载，但分析层不直接消费“模糊 tags”，而是投影为明确维度：
  - 行业（Industry）：以申万（SW）口径为主，支持多级（与 SW 层级一致），用于行业敞口/风险敞口/归因等“可复算口径”。
  - 主题（Theme）：多对多标签，用于机会发现/叙事跟踪/主题敞口；允许更高频变动。
  - Namespace 约定（草案）：行业 `ind:sw:l1:<name>` / `ind:sw:l2:<name>` / `ind:sw:l3:<name>`；主题 `theme:<provider>:<name>`（例如 `theme:ths:<name>`）；用户自定义标签使用 `user:<name>`（不作为默认行业口径）。
