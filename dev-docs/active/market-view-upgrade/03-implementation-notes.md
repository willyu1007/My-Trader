...
- 2026-02-09：按“目标池为唯一主入口”做无破坏收敛：数据管理页隐藏“注册标的”区块并关闭相关自动刷新；同时将 `includeRegistryAutoIngest` 默认与读写流程统一收敛为 `false`，避免旧配置继续通过注册标的影响目标池。
- 2026-02-09：目标池编辑右侧“差异预览”增强交互：`新增/移除/变化来源` 三个分组均支持独立折叠/展开；并将原“来源变化”命名调整为“变化来源”以匹配术语。
- 2026-02-09：目标池编辑左右面板增加可拖拽调宽（按住中间分隔线），支持键盘 `←/→` 微调（`Shift` 加速）；宽度比例持久化到 `localStorage`，并设置最小/最大阈值防止极端布局。
