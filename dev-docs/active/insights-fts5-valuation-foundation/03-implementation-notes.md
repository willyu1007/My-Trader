...
- 版式参数：
  - 左右列宽比固定为 `2:1`；
  - 总高度固定为 `760px`；
  - 右上区域高度固定为 `280px`，右下区域占据剩余高度。

## 2026-03-02 (生成页宽比微调 5:3)
- `生成` 页左右分栏比例从 `2:1` 调整为 `5:3`，保持其余固定高度参数不变。

## 2026-03-02 (观点草稿操作按钮文案与对齐)
- 将 `去管理/创建观点` 按钮文案改为 `管理/创建`。
- 按钮改为同一操作组容器，固定同高同宽，避免换行拆分导致上下堆叠。
